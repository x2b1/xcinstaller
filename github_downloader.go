...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("TESTCORD_DEV_INSTALL") == "1"
	...
	TestCordFile := TestCordDirectory

	stat, err := os.Stat(TestCordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		TestCordFile = path.Join(TestCordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(TestCordFile)
	if err != nil {
		return
	}
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(TestCordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", TestCordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", TestCordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(TestCordDirectory)
